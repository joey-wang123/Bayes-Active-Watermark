...
def test_teacher(args, device, teacher = None, test_loader = None):
    correct = 0
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader):
            data, target = data.to(device), target.to(device)
...
def main():
    ...
    if args.model == 'resnet34_8x':
        teacher = network.resnet_8x_w.ResNet34_8x(args, num_classes=num_classes)
    ...
    watermarknet = Res2Net(epsilon=0.50, hidden_planes=2, batch_size=watermarknet_batch_size).train().to(device)
    watermarknet_posterior = Res2Net_weight(epsilon=0.50, hidden_planes=2, batch_size=watermarknet_batch_size).train().to(device)
    teacher.watermarknet = watermarknet_posterior

    teacher.eval()
    teacher = teacher.to(device)

    print(f"\n\t\tTraining with {args.model} as a Target\n") 
    acc = test_teacher(args, device, teacher = teacher, test_loader = test_loader)
