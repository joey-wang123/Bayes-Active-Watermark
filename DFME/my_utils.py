...
def add_noise_to_weights(m):
    ...
    with torch.no_grad():
        # if hasattr(model_copy, 'weight'):
        #     model_copy.weight.add_(torch.randn(model_copy.weight.size()) * delta)
        for param in model_copy.parameters():
            param.add_(torch.randn(param.size()) * delta)

    for t, s in zip(m.parameters(), model_copy.parameters()):
        norm = torch.norm(t-s)
        print('norm', norm)
